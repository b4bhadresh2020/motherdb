        ...
        if (isset($_GET['apikey'])) {
            ...
            if ($_GET['apikey'] != '') {
                ...
                if ($getLiveDeliveryCount > 0) {
                    ...
                    $seCheckApiKey = array(
                        "45754a36795461653447446c485a5152587177342f413d3d",
                        "313949444473776263447432795775364964796d75673d3d",
                        "646c4b47765239794b44616a565762616445786366413d3d"
                    );
                            ...
                            if (isset($_GET[$identifier])) {
                                ...
                                if ($_GET[$identifier] != '') {
                                    ...
                                    if ($notToCheckFuther == 0) {
                                        ...
                                        } else if (strpos($emailAddressChunk[1], HOTMAIL_DOMAIN) !== false && in_array($_GET['apikey'], $seCheckApiKey)) {
                                            $notToCheckFuther = 14; // HOTMAIL MX Block
                                        } else if (strpos($emailAddressChunk[1], OUTLOOK_DOMAIN) !== false && in_array($_GET['apikey'], $seCheckApiKey)) {
                                            $notToCheckFuther = 15; // OUTLOOK MX Block
                                        } else if (strpos($emailAddressChunk[1], LIVE_DOMAIN) !== false && in_array($_GET['apikey'], $seCheckApiKey)) {
                                            $notToCheckFuther = 16; // LIVE MX Block
                                        } else if (strpos($emailAddressChunk[1], MSN_DOMAIN) !== false && in_array($_GET['apikey'], $seCheckApiKey)) {
                                            $notToCheckFuther = 17; // MSN MX Block
                                        }
                                    ...
                                    } else if ($notToCheckFuther == 14) {
                                        //data save to live_delivery_data table
                                        $isFail            = 1;
                                        $sucFailMsgIndex   = 25; // Hotmail MX Block
                                        $response['error'] = 'Hotmail MX Block';
                                    } else if ($notToCheckFuther == 15) {
                                        //data save to live_delivery_data table
                                        $isFail            = 1;
                                        $sucFailMsgIndex   = 26; // Outlook MX Block
                                        $response['error'] = 'Outlook MX Block';
                                    } else if ($notToCheckFuther == 16) {
                                        //data save to live_delivery_data table
                                        $isFail            = 1;
                                        $sucFailMsgIndex   = 27; // Live MX Block
                                        $response['error'] = 'Live MX Block';
                                    } else if ($notToCheckFuther == 17) {
                                        //data save to live_delivery_data table
                                        $isFail            = 1;
