...
class Cron_marketing_platform_delay_user extends CI_Controller
{
    ...
    public function index() {       
        ...
        $is_single = FALSE;
        $userData = JoinData(MARKETING_PLATFORM_DELAY_USER_DATA,$condition,LIVE_DELIVERY_DATA,"liveDeliveryDataId","liveDeliveryDataId","left",$is_single,array(),"","");

        $this->db->select('marketing_platform_delay_user_data.id,live_delivery_data.liveDeliveryDataId,live_delivery_data.apikey,firstName,lastName,emailId,address,postCode,city,live_delivery_data.country,phone,gender,birthdateDay,birthdateMonth,birthdateYear,age,ip,tag,sucFailMsgIndex,providerId,live_delivery_data.groupName,live_delivery_data.keyword,isDuplicate');
        $this->db->from(MARKETING_PLATFORM_DELAY_USER_DATA);
        $this->db->join(LIVE_DELIVERY_DATA,'marketing_platform_delay_user_data.liveDeliveryDataId=live_delivery_data.liveDeliveryDataId');
