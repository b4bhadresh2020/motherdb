...
class Cron_sendpulse_delay_user extends CI_Controller
{
    ...
    public function index() {       
        ...
        $is_single = FALSE;
        $userData = JoinData(SENDPULSE_DELAY_USER_DATA,$condition,LIVE_DELIVERY_DATA,"liveDeliveryDataId","liveDeliveryDataId","left",$is_single,array(),"","");

        $this->db->select('sendpulse_delay_user_data.id,live_delivery_data.liveDeliveryDataId,live_delivery_data.apikey,firstName,lastName,emailId,address,postCode,city,live_delivery_data.country,phone,gender,birthdateDay,birthdateMonth,birthdateYear,age,ip,tag,sucFailMsgIndex,providerId,live_delivery_data.groupName,live_delivery_data.keyword,isDuplicate');
        $this->db->from(SENDPULSE_DELAY_USER_DATA);
        $this->db->join(LIVE_DELIVERY_DATA,'sendpulse_delay_user_data.liveDeliveryDataId=live_delivery_data.liveDeliveryDataId');
        ...
        foreach($userData as $user){   
            ...
            if($emailAddressChunk[1] == TELIA_DOMAIN || $emailAddressChunk[1] == LUUKKU_DOMAIN || (startsWith($emailAddressChunk[1],PP_DOMAIN_START) && endsWith($emailAddressChunk[1],PP_DOMAIN_END)) || (strpos($emailAddressChunk[1], YAHOO_DOMAIN) !== false && $country == 'SE') || (strpos($emailAddressChunk[1], ICLOUD_DOMAIN) !== false) || (strpos($emailAddressChunk[1], GMX_DOMAIN) !== false)){
                ...
                // Update response in sendpulse queue user data table
                ...
                ManageData(SENDPULSE_DELAY_USER_DATA, $queueCondition, $queueUpdateArr, $is_insert);

                // Update response in sendpulse history data.
                $historyData = array(       
                ...
                if(!array_key_exists($user['providerId'],$isDuplicate) || (array_key_exists($user['providerId'],$isDuplicate) && $user['sucFailMsgIndex'] == 1)){
                    ...
                    $response = $this->mdl_sendpulse->AddEmailToSendpulseSubscriberList($user,$user['providerId']);
                }else{
