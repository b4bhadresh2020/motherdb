<script type="text/javascript">
		...
		$.ajax({
			...
			success: function(response) {
				...
				if (providerId == 1) {
					...
					sendEmailToExpertSender(apiData, provider, groupName, keyword);
				} else if (providerId == 15) {
					sendEmailToCleverReach(apiData, provider, groupName, keyword);
				} else if (providerId == 16) {
					sendEmailToOmnisend(apiData, provider, groupName, keyword);
				}
			}
		});
	}
	...
	var sendEmailToExpertSenderCount = 0;
	var sendEmailToCleverReachCount = 0;
	var sendEmailToOmnisendCount = 0;
	...
	function sendEmailToMailerlite(apiData, provider, groupName, keyword) {
		var apiDataDetail = apiData[sendEmailToMailerliteCount];

		$.ajax({
			url: BASEPATH + 'repost/addDataToMailerlite',
			type: 'post',
			data: {
				apiDataDetail: apiDataDetail,
				provider: provider,
			...
			success: function(response) {
				...
				sendEmailToMailerliteCount++;
				if (sendEmailToMailerliteCount < apiData.length) {
					sendEmailToMailerlite(apiData, provider, groupName, keyword);
				}
	...
	function sendEmailToMailjet(apiData, provider, groupName, keyword) {
		var apiDataDetail = apiData[sendEmailToMailjetCount];

		$.ajax({
			url: BASEPATH + 'repost/addDataToMailjet',
			type: 'post',
			data: {
				apiDataDetail: apiDataDetail,
				provider: provider,
	...
	function sendEmailToMarketingPlatform(apiData, provider, groupName, keyword) {
		var apiDataDetail = apiData[sendEmailToMarketingPlatformCount];

		$.ajax({
			url: BASEPATH + 'repost/addDataToMarketingPlatform',
			type: 'post',
			data: {
				apiDataDetail: apiDataDetail,
				provider: provider,
	...
	function sendEmailToActiveCampaign(apiData, provider, groupName, keyword) {
		...
		$.ajax({
			url: BASEPATH + 'repost/addDataToActiveCampaign',
			type: 'post',
			data: {
				apiDataDetail: apiDataDetail,
				provider: provider,
			...
			success: function(response) {
				...
				sendEmailToActiveCampaignCount++;
				if (sendEmailToActiveCampaignCount < apiData.length) {
					sendEmailToActiveCampaign(apiData, provider, groupName, keyword);
				}
	...
	function sendEmailToExpertSender(apiData, provider, groupName, keyword) {
		var apiDataDetail = apiData[sendEmailToExpertSenderCount];

		$.ajax({
			url: BASEPATH + 'repost/addDataToExpertSender',
			type: 'post',
			data: {
				apiDataDetail: apiDataDetail,
				provider: provider,
			...
			success: function(response) {
				...
				sendEmailToExpertSenderCount++;
				if (sendEmailToExpertSenderCount < apiData.length) {
					sendEmailToExpertSender(apiData, provider, groupName, keyword);
				}
	...
	function sendEmailToCleverReach(apiData, provider, groupName, keyword) {
		var apiDataDetail = apiData[sendEmailToCleverReachCount];

		$.ajax({
			url: BASEPATH + 'repost/addDataToCleverReach',
			type: 'post',
			data: {
				apiDataDetail: apiDataDetail,
				provider: provider,
			...
			success: function(response) {
				...
				sendEmailToCleverReachCount++;
				if (sendEmailToCleverReachCount < apiData.length) {
					sendEmailToCleverReach(apiData, provider, groupName, keyword);
				}
	...
	function sendEmailToOmnisend(apiData, provider, groupName, keyword) {
		var apiDataDetail = apiData[sendEmailToOmnisendCount];
		
		$.ajax({
			url: BASEPATH + 'repost/addDataToOmnisend',
			type: 'post',
			data: {
				apiDataDetail: apiDataDetail,
				provider: provider,
			...
			success: function(response) {
				...
				sendEmailToOmnisendCount++;
				if (sendEmailToOmnisendCount < apiData.length) {
					sendEmailToOmnisend(apiData, provider, groupName, keyword);
				}
